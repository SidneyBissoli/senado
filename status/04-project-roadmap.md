...
| Primeira função retornando dados reais | 02 | `sen_get()` operacional com retry/cache |
...
- [x] **Nome do pacote:** `senado` (curto, limpo, memorizável — confirmar disponibilidade no CRAN)
- [x] **Prefixo das funções:** `sen_` (ex: `sen_senators()`, `sen_votes()`) — curto, bilíngue (PT *senado* / EN *senate*), segue padrão do ecossistema (`str_`, `gs4_`, `fct_`)
- [x] **Dependências core (mínimas):**
...
- [x] **Dependências sugeridas (Suggests):**
  - `testthat` (≥ 3.0.0)
...
- [x] **Infraestrutura compartilhada:** começar dentro de `senado` em `R/utils-api.R`; extrair para pacote separado (`apigateway`) somente quando outro pacote (healthbR, educabR, welfarebR) precisar
- [x] **Formato de saída:** todas as funções retornam `tibble` com colunas em `snake_case`
- [x] **Licença:** MIT (consistente com ecossistema rOpenSci)
- [x] **Versão mínima do R:** ≥ 4.1.0 (para suporte nativo ao pipe `|>`)
...
- [ ] Estrutura de pastas criada:
  ...
  │   ├── utils-api.R             # sen_get(), retry, cache, user-agent
...
**Objetivo:** Implementar o `sen_get()` e toda a infraestrutura que garante robustez no acesso à API do Senado.
...
- [ ] **`R/utils-api.R`** — Função central `sen_get()`:
...
- [ ] **`R/utils-cache.R`** — Sistema de cache:
  ...
  - [ ] Função `sen_cache_clear()` para limpar cache manualmente
  - [ ] Opção global `sen_use_cache` (TRUE/FALSE) via `options()`
...
- [ ] **Máquina (Claude):** Todos os testes passando; `devtools::check()` 0/0/0; `sen_get("senador/lista/atual")` retorna tibble.
- [ ] **Humano (Sidney):** Testar `sen_get()` interativamente no RStudio contra 5+ endpoints diferentes; validar que retry funciona (ex: desconectar internet temporariamente).
...
1. **Uma função por ação semântica** (não por endpoint). Ex: `sen_senators()` agrega lista + filtro.
...
- [ ] `sen_legislatures()` — Lista de legislaturas (número, datas, senadores)
- [ ] `sen_current_legislature()` — Legislatura atual (shortcut)
- [ ] `sen_parties()` — Partidos com representação no Senado
- [ ] `sen_states()` — UFs com senadores
- [ ] `sen_bill_types()` — Tipos de matéria legislativa (PEC, PL, PLS, etc.)
- [ ] `sen_proceeding_types()` — Tipos de tramitação

### Módulo 3.2 — Senadores (`R/senators.R`)

- [ ] `sen_senators()` — Lista de senadores (com filtros: legislature, state, party, status)
- [ ] `sen_senator()` — Detalhes de um senador por código
- [ ] `sen_senator_votes()` — Votações nominais de um senador
- [ ] `sen_senator_bills()` — Matérias de autoria de um senador
- [ ] `sen_senator_committees()` — Comissões de que um senador é membro
- [ ] `sen_senator_mandates()` — Histórico de mandatos

### Módulo 3.3 — Matérias legislativas (`R/bills.R`)

- [ ] `sen_bills()` — Busca de matérias (com filtros: year, type, author, keyword, status)
- [ ] `sen_bill()` — Detalhes de uma matéria por código
- [ ] `sen_bill_text()` — Texto(s) da matéria (inteiro teor, emendas, substitutivos)
- [ ] `sen_bill_proceedings()` — Tramitação detalhada (timeline)
- [ ] `sen_bill_votes()` — Votações relacionadas à matéria
- [ ] `sen_bill_authors()` — Autores e coautores

### Módulo 3.4 — Votações (`R/votes.R`)

- [ ] `sen_votes()` — Lista de votações (com filtros: year, date_range, bill_type)
- [ ] `sen_vote()` — Detalhes de uma votação por código
- [ ] `sen_vote_roll_call()` — Votos nominais (quem votou o quê)
- [ ] `sen_recent_votes()` — Votações dos últimos N dias

### Módulo 3.5 — Comissões (`R/committees.R`)

- [ ] `sen_committees()` — Lista de comissões (com filtro: type = permanent/temporary)
- [ ] `sen_committee()` — Detalhes de uma comissão
- [ ] `sen_committee_members()` — Membros de uma comissão
- [ ] `sen_committee_meetings()` — Reuniões agendadas/realizadas

### Módulo 3.6 — Plenário e agenda (`R/plenary.R`)

- [ ] `sen_agenda()` — Agenda do plenário (com filtros: date, date_range)
- [ ] `sen_sessions()` — Sessões plenárias realizadas
...
- [ ] **`R/ecidadania.R`** — Funções de acesso:
  - [ ] `sen_consultations()` — Consultas públicas (com filtros: status, theme, date_range)
  - [ ] `sen_consultation()` — Detalhes de uma consulta (votos sim/não, total)
  - [ ] `sen_ideas()` — Ideias legislativas (com filtros: status, keyword, sort_by)
  - [ ] `sen_idea()` — Detalhes de uma ideia (apoios, status)
  - [ ] `sen_events()` — Eventos interativos (audiências públicas, sabatinas, lives)
  - [ ] `sen_event()` — Detalhes de um evento
...
- [ ] `sen_senator_profile()` — Perfil consolidado de um senador: dados biográficos + comissões + votações + matérias de autoria, tudo em uma lista nomeada de tibbles
- [ ] `sen_bill_timeline()` — Timeline completa de uma matéria: autoria → tramitação → votações → resultado, em tibble com coluna `event_type`
- [ ] `sen_party_cohesion()` — Índice de coesão partidária por votação ou período (% de votos alinhados com a maioria do partido)
- [ ] `sen_vote_matrix()` — Matriz de votação (senadores × votações) em formato wide, pronta para análise (NOMINATE, ideal points)
- [ ] `sen_attendance()` — Taxa de presença por senador/período

### Módulo 5.2 — Dados pré-construídos (`data-raw/` e `data/`)

- [ ] `sen_historical_senators` — Dataset com todos os senadores desde 1991 (tibble no pacote)
- [ ] `sen_historical_votes` — Dataset com todas as votações nominais desde 1991
...
- [ ] `sen_predict_approval()` — Estima a probabilidade de aprovação de uma matéria com base em:
...
- [ ] `sen_clean_names()` — Wrapper para normalizar nomes de colunas de qualquer output (já feito internamente, mas exposto para usuário avançado)
- [ ] `sen_merge_votes()` — Merge entre votações e dados biográficos dos senadores (join por código)
- [ ] `sen_pivot_votes()` — Pivota votos nominais de long → wide (matriz senador × votação)
...
3. **Package design** — Decisões arquiteturais: `sen_get()`, camada de cache, parsing tidy, minimal dependencies.
...
|---|---|---|---|
| **Reference** | `sen_legislatures()` | 03 | REST `/legislatura` |
| | `sen_current_legislature()` | 03 | REST `/legislatura/atual` |
| | `sen_parties()` | 03 | REST `/partidos` |
| | `sen_states()` | 03 | REST `/ufs` |
| | `sen_bill_types()` | 03 | REST `/tipos-materia` |
| | `sen_proceeding_types()` | 03 | REST `/tipos-tramitacao` |
| **Senators** | `sen_senators()` | 03 | REST `/senador/lista` |
| | `sen_senator()` | 03 | REST `/senador/{codigo}` |
| | `sen_senator_votes()` | 03 | REST `/senador/{codigo}/votacoes` |
| | `sen_senator_bills()` | 03 | REST `/senador/{codigo}/materias` |
| | `sen_senator_committees()` | 03 | REST `/senador/{codigo}/comissoes` |
| | `sen_senator_mandates()` | 03 | REST `/senador/{codigo}/mandatos` |
| **Bills** | `sen_bills()` | 03 | REST `/materia/pesquisa` |
| | `sen_bill()` | 03 | REST `/materia/{codigo}` |
| | `sen_bill_text()` | 03 | REST `/materia/{codigo}/textos` |
| | `sen_bill_proceedings()` | 03 | REST `/materia/{codigo}/tramitacao` |
| | `sen_bill_votes()` | 03 | REST `/materia/{codigo}/votacoes` |
| | `sen_bill_authors()` | 03 | REST `/materia/{codigo}/autores` |
| **Votes** | `sen_votes()` | 03 | REST `/votacao/lista` |
| | `sen_vote()` | 03 | REST `/votacao/{codigo}` |
| | `sen_vote_roll_call()` | 03 | REST `/votacao/{codigo}/votos` |
| | `sen_recent_votes()` | 03 | REST `/votacao/recentes` |
| **Committees** | `sen_committees()` | 03 | REST `/comissao/lista` |
| | `sen_committee()` | 03 | REST `/comissao/{codigo}` |
| | `sen_committee_members()` | 03 | REST `/comissao/{codigo}/membros` |
| | `sen_committee_meetings()` | 03 | REST `/comissao/{codigo}/reunioes` |
| **Plenary** | `sen_agenda()` | 03 | REST `/agenda/plenario` |
| | `sen_sessions()` | 03 | REST `/sessao/lista` |
